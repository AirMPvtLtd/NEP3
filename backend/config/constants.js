 ...
 * Central location for all application constants
 * Updated with: Subject selection, Custom topics, Auto difficulty, Question counts
...
const CHALLENGE_DIFFICULTY = {
  ...
  MEDIUM: 'medium',
  HARD: 'hard',
  AUTO: 'auto'  // NEW: Ferrari Engine auto-selects
};

// Difficulty mode (NEW)
const DIFFICULTY_MODE = {
  MANUAL: 'manual',  // User selected difficulty
  AUTO: 'auto'       // Ferrari Engine selected difficulty
};

// Question count options (NEW)
const QUESTION_COUNT_OPTIONS = [3, 5, 7, 10];

// Topic types (NEW)
const TOPIC_TYPES = {
  SIMULATION: 'simulation',  // From predefined simulation list
  CUSTOM: 'custom'           // User-provided custom topic
};

// Challenge types (existing)
const CHALLENGE_TYPES = {
  MCQ: 'MCQ',
  NUMERICAL: 'NUMERICAL',
  SHORT_ANSWER: 'SHORT_ANSWER',
...
const CHALLENGE_LIMITS = {
  ...
  MIN_PASSING_SCORE: 70,
  MIN_QUESTIONS: 3,          // NEW
  MAX_QUESTIONS: 10,         // NEW
  DEFAULT_QUESTIONS: 5       // NEW
};
...
const PID_CONTROLLER = {
  ...
  INTEGRAL_MAX: 50,
  INTEGRAL_MIN: -50,
  SETPOINT: 70  // NEW: Target performance
};
...
// Ferrari Engine difficulty prediction weights (NEW)
const FERRARI_ENGINE = {
  ENABLED: true,
...
const isValidSubject = (subject) => {
  return Object.values(CHALLENGE_SUBJECTS).includes(subject);
};
...
const isValidDifficulty = (difficulty) => {
  return Object.values(CHALLENGE_DIFFICULTY).includes(difficulty);
};

/**
 * Check if question count is valid
 */
const isValidQuestionCount = (count) => {
  return QUESTION_COUNT_OPTIONS.includes(count);
};

/**
 * Validate custom topic
 */
const isValidCustomTopic = (topic) => {
  if (!topic || typeof topic !== 'string') return false;
  const length = topic.trim().length;
  return length >= CUSTOM_TOPIC.MIN_LENGTH && length <= CUSTOM_TOPIC.MAX_LENGTH;
};

/**
 * Get simulations by subject
 */
const getSimulationsBySubject = (subject) => {
  if (subject === 'physics') return PHYSICS_SIMULATIONS;
  if (subject === 'mathematics') return MATH_SIMULATIONS;
  return [];
};
 ...
 */
const isValidSimulation = (simulationType) => {
  return [...PHYSICS_SIMULATIONS, ...MATH_SIMULATIONS].includes(simulationType);
};
...
module.exports = {
  ...
  // Challenges (UPDATED)
  CHALLENGE_SUBJECTS,        // NEW
  CHALLENGE_DIFFICULTY,      // UPDATED (added AUTO)
  DIFFICULTY_MODE,           // NEW
  QUESTION_COUNT_OPTIONS,    // NEW
  TOPIC_TYPES,               // NEW
  CHALLENGE_TYPES,
  CHALLENGE_LIMITS,          // UPDATED (added MIN/MAX/DEFAULT questions)
  CHALLENGE_POINTS,
  CUSTOM_TOPIC,              // NEW
  ...
  // Algorithms (UPDATED)
  KALMAN_FILTER,
  PID_CONTROLLER,            // UPDATED (added SETPOINT)
  META_LEARNING,
  FERRARI_ENGINE,            // NEW
